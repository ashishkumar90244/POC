...
app.get('/todo',function(req,res){
    ...
    fs.readFile('./treasure.txt',"utf-8",(err,data)=>{
        ...
        data = JSON.parse(data);
        if(!(req.session.username in data))
            res.render('todo',{username:req.session.username ,todos:[]});
        else 
        res.render('todo',{username:req.session.username ,todos:data[req.session.username]});
    })
...
app.get('/plan',function(req,res){
    ...
    fs.readFile('./treasure.txt',"utf-8",(err,data)=>{
        ...
        data = JSON.parse(data);
        if(!(req.session.username in data))
            res.render('plan',{username:req.session.username ,todos:[]});
        else 
        res.render('plan',{username:req.session.username ,todos:data[req.session.username]});
    })
...
app.get('/plan-data', (req, res) => {
    

    fs.readFile('./userBase/plan.txt', 'utf8', (err, data) => {
        if (err) {
